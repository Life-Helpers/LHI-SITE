...
export function EventDateTile({ event, size = "md", className = "" }: { event: Pick<CalendarEvent, "start" | "area">; size?: "md" | "lg"; className?: string }) {
  const month = monthShort(event.start);
  const day = Number(event.start.slice(8, 10));
  const lhi = event.area === "lhi";
  return (
    <div
      aria-hidden="true"
      className={`flex shrink-0 flex-col items-center justify-center overflow-hidden rounded-2xl border text-center ${
        lhi ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground"
      } ${size === "lg" ? "h-24 w-24" : "h-16 w-16"} ${className}`}
    >
      <span className={`w-full py-0.5 text-[10px] font-semibold uppercase tracking-widest ${lhi ? "bg-black/15" : "bg-primary text-primary-foreground"}`}>{month}</span>
