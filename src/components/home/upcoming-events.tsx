...
import {
  countdownLabel,
  EventDateTile,
} from "@/components/events/event-date-tile";
import {
  daysUntil,
  formatEventDate,
  OBSERVANCE_AREAS,
    ...
    >
      <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2
            id="upcoming-heading"
            className="mt-2 font-serif-display text-3xl font-light sm:text-4xl"
          >
        ...
        >
          <div className="flex items-start gap-5">
            ...
            <div className="min-w-0">
              <p
                className={`text-[11px] font-semibold uppercase tracking-[0.25em] ${lhi ? "text-white" : "text-accent"}`}
              >
                {countdownLabel(daysUntil(today, next.start))} ·{" "}
                {OBSERVANCE_AREAS[next.area].label}
              </p>
              <h3 className="mt-2 font-serif-display text-3xl font-light leading-tight sm:text-4xl">
                {next.title}
              </h3>
              <p
                className={`mt-1 text-sm ${lhi ? "text-white" : "text-muted-foreground"}`}
              >
          ...
          <p
            className={`mt-5 max-w-xl leading-relaxed ${lhi ? "text-white" : "text-foreground/80"}`}
          >
            {next.description}
          </p>
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            <Link
              href={`/events#${next.id}`}
              className={`text-sm font-semibold hover:underline ${lhi ? "text-white" : "text-primary"}`}
            >
              Learn more →
            </Link>
          </div>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {countdownLabel(daysUntil(today, e.start))}
                </p>
              </div>
            </li>
          ))}
